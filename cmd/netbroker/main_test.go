...
func TestWireProtocolRejectsUnrecognizedFields(t *testing.T) {
	broker := testBroker(t)
	response := exchange(t, broker, `{"operation":"dry_run","request":{"job_id":"job_wire_0001","platform":"nostr","endpoint_id":"aws-fr-1","address":"203.0.113.1"}}`)
...
func TestWireProtocolDryRunKnownEndpoint(t *testing.T) {
	broker := testBroker(t)
	response := exchange(t, broker, `{"operation":"dry_run","request":{"job_id":"job_wire_0002","platform":"nostr","endpoint_id":"aws-fr-1"}}`)
	if !response.OK {
